...
def Day16_Part2(filename='Inputs/Day16_Inputs.txt'):
    ...
    if length_id == 0:
        ...
        binary_data = binary_data[22:]
        binary_data, sub_length, sub_packet = process_set_subpacket(binary_data, length, False, level+1)
        packet += sub_packet
    elif length_id == 1:
        number = int(binary_data[7:18], base=2)
        binary_data = binary_data[18:]
        binary_data, sub_length, sub_packet = process_set_subpacket(binary_data, False, number, level+1)
        packet += sub_packet
...
def process_set_subpacket(binary_data, max_len, max_num, level):
    """
    Decodes the sub-packets given in a packet with a set total length in bits (length type
    ID == 0) or a set number of contained sub-packets (length type ID == 1).

    Parameters
    ----------
    binary_data : str
        The remaining binary data from the outermost packet that is still to be parsed.
    max_len : int or False
        If the packet length type ID == 0, this should be the total length in bits of the
        sub-packets in this packet.
        If the packet length type ID == 1, this should be False.
    max_num : int or False
        If the packet length type ID == 1, this should be the total number of sub-packets
        in this packet.
        If the packet length type ID == 0, this should be False.
    level : int
    ...
    """
    curr_length, curr_number, packet = 0, 0, []
    while (curr_length < max_len or not max_len) and (curr_number < max_num or not max_num):
        type_id = int(binary_data[3:6], base=2)
        curr_length += 6
        if type_id == 4:
            ...
            if length_id == 0:
                ...
                binary_data = binary_data[22:]
                curr_length += 15
                binary_data, sub_length, sub_packet_contents = process_set_subpacket(binary_data, length, False, level+1)
    
            elif length_id == 1:
                number = int(binary_data[7:18], base=2)
                binary_data = binary_data[18:]
                curr_length += 11
                binary_data, sub_length, sub_packet_contents = process_set_subpacket(binary_data, False, number, level+1)
